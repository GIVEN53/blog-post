...
def create_content(title: str, summary: str) -> str:
    ...
    for i in range(len(contents)):
        code_block = re.search(r'<code\s+class="([^"]+)"', contents[i]) # (2)
        if code_block:
            language = code_block.group(1)
...
* (1) HTML의 코드블럭은 `<pre><code>...</code></pre>`이므로 `<pre>`를 기준으로 split한다.
* (2) 티스토리는 선언한 프로그래밍 언어를 `<code>` 태그의 class 속성에 정의하고 있기 때문에 정규식으로 해당 부분을 찾는다.  
